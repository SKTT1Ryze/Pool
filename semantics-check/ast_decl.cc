...
ClassDecl::ClassDecl(Identifier *n, NamedType *ex, List<NamedType*> *imp, List<Decl*> *m) : Decl(n) {
  ...
  this->inherit = ex;
  if (this->inherit) this->inherit->SetParent(this);
...
void ClassDecl::CheckDeclError() {
  ...
  NamedType *ex = this->inherit;
  while (ex)
    {
      const char *name = ex->GetID()->GetName();
      if (name)
	{
	  Node *node = Pool::sym_table->Lookup(name);
	  if (node == NULL)
	    {
  ...
  if (this->implements)
      ...
      for (int i = 0; i < this->implements->NumElements(); i++)
	{
	  ...
	  if (id)
	    {
	      Node *node = Pool::sym_table->Lookup(id->GetName());
      ...
      if (extends)
	    ...
	    return true;
          else
           {
             if (name)
               {
                 Decl *exdecl = Pool::sym_table->Lookup(name);
...
void LifeDecl::CheckDeclError() {
  ...
  while (ex)
      ...
      if (name)
	{
	  ...
	  else if (typeid(*node) == typeid(LifeDecl))
	    {
	      LifeDecl *base = dynamic_cast<LifeDecl*>(node);
	      List<Decl*> *base_members = base->members;
	      List<Decl*> *inherited = new List<Decl*>;
	      // check the declaration of base class against derived class symbol table
	      if (base_members)
...
bool ClassDecl::IsCompatibleWith(Decl *decl)
{
  ...
  if (typeid(*decl) == typeid(LifeDecl))
    {
      LifeDecl *cldecl = dynamic_cast<LifeDecl*>(decl);
      // is B a base class of A
      if (extends)
        {
          const char *name = extends->GetTypeName();
	    ...
	    return true;
          else
           {
             if (name)
               {
                 Decl *exdecl = Pool::sym_table->Lookup(name);
                 if (exdecl && typeid(*exdecl) == typeid(LifeDecl))
                   return dynamic_cast<LifeDecl*>(exdecl)->IsCompatibleWith(decl);
               }
           }
        }
    }
