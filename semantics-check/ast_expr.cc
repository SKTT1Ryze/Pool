...
void EqualityExpr::CheckStatements() {
  ...
  if (lt && rt)
    {
      Decl *ldecl = Pool::sym_table->Lookup(lt);
      Decl *rdecl = Pool::sym_table->Lookup(rt);
...
void FieldAccess::CheckStatements() {
  ...
  if (this->base)
      ...
      if (name)
	{
	  ...
	  if (cldecl == NULL)
	    {
	      if ((cldecl = Pool::sym_table->Lookup(name)) != NULL) // look up global symbol table
	        {
	          decl = this->field->CheckIdDecl(cldecl->GetSymTable(), this->field->GetName());
	          if ((decl != NULL) && (typeid(*decl) == typeid(VarDecl)))
  ...
  if (formals_num != args_num)
      ...
      for (int i = 0; i < formals_num; i++)
	{
	  ...
	  const char *given = expr->GetTypeName();
          ...
          if (given && expected)
            {
              Decl *gdecl = Pool::sym_table->Lookup(given);
              Decl *edecl = Pool::sym_table->Lookup(expected);
  ...
  if (this->base)
    {
      ...
      if (name)
        {
          if ((decl = Pool::sym_table->Lookup(name)) != NULL)
	    {
...
void SpawnExpr::CheckStatements() {
  if (this->cType)
    {
      const char *name = this->cType->GetTypeName();
      if (name)
